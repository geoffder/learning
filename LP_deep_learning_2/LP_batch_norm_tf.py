...
class HiddenLayerBatchNorm(object):
    def __init__(self, M1, M2, f):
        ...
        self.running_mean = tf.Variable(
            np.zeros(M2).astype(np.float32), trainable=False)
    ...
    def forward(self, X, is_training, decay=0.9):
        ...
        if is_training:
            ...
            with tf.control_dependencies([update_running_mean,
                                         update_running_var]):
...
class ANN(object):
    ...
    def fit(self, X, Y, Xtest, Ytest, activation=tf.nn.relu,
            learning_rate=1e-2, epochs=15, batch_sz=100, print_period=100,
            show_fig=True):
        ...
        # train_op = tf.train.RMSPropOptimizer(learning_rate, decay=0.99,
        #                                      momentum=0.9).minimize(cost)
        train_op = tf.train.MomentumOptimizer(
            learning_rate, momentum=0.9, use_nesterov=True).minimize(cost)
        ...
        for i in range(epochs):
            ...
            for j in range(n_batches):
                ...
                c, _, lgts = self.session.run(
                    [cost, train_op, logits], feed_dict={tfX: Xbatch,
                                                         tfY: Ybatch}
                )
                costs.append(c)
                if (j+1) % print_period == 0:
                    ...
                    # print(
                    #     'dbg:',
                    #     self.session.run(self.layers[0].running_mean).sum()
                    # )

        print("Train acc:", self.score(X, Y),
              "Test acc:", self.score(Xtest, Ytest))
