...
from LP_greedy_ae_pretrain_tf import DNN
...
class RBM(object):
    ...
    def build(self, D, M):
        ...
        #     dtype=tf.float32
        # )
        'Manual sample generation used (below), Bernoulli API was changing'
        'Generate random uniform distribution'
        r = tf.random_uniform(shape=tf.shape(p_h_given_v))
        'Then get H, which is 0 where r > phv and 1 where r < phv'
        H = tf.to_float(r < p_h_given_v)  # returned bool is changed to float
        ...
        '''
        X_sample is the final equation/definition that ties all of the above
        together, and is passed to the objective function. To get X_sample,
        p_h_given_v, H, and p_v_given_h must all be calculated. Thus then are
        built in to the graph that is run when executing train_op, which the
        optimized minimizing the objective function.
        '''
    ...
    def free_energy(self, V):
        'F(v) = -b.T @ V - sum(log(1 + exp(V.T @ W + c)), axis=1)'
    ...
    def forward_hidden(self, X):
        'Used for forward propagation once the RBMs are stacked'
