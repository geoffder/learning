...
def buildRequests(raceStart):
    ...
    endpoints = ["https://min-api.cryptocompare.com/data/histominute?fsym=%s&tsym=USD&limit=999&toTs=%d" % (
...
def getRacePrices(raceStart):
    endpoints, headers = buildRequests(raceStart)
    loop = asyncio.get_event_loop()
