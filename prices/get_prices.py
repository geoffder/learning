...
def buildRequests():
    ...
    # 2000 is max number of minutes at a time,
    # get more by repeating with different #&toTs=
    endpoints = ["https://min-api.cryptocompare.com/data/histominute"
                 + "?fsym=%s&tsym=USD&limit=2000&toTs=%d" % (s, t)
                 for s in tickers for t in toTime]
    headers = {'authorization': 'e35795772b742f4c32495080678cdab0988'
                                + '17bcab591b2f44d0aae61c2d32ef3'}
...
def createSamples(X):
    # sampling variables
    hist = 300  # minutes of data per sample (leading up to prediction)
    raceLen = 60
    advanceBet = 14  # number of minutes in advance
    jitterRange = 6  # some randomness in when price is locked
    # the idea is to make the model more resilient and approximate the races
    ...
    for t in range(hist, X.shape[0]-raceLen-advanceBet-10, 370):
        ...
        for i in range(3):
            ...
            priceEnd = X[t+raceLen+advanceBet+jitter[1],
                         int(i*len(metricKeys)+1)]
...
def getRacePrices(raceStart):
    ...
    endpoints = [
        ...
        + "?fsym=%s&tsym=USD&limit=999&toTs=%d" % (s, lastTime)
        for s in tickers
    ]
    headers = {'authorization': 'e35795772b742f4c32495080678cdab0988'
                                + '17bcab591b2f44d0aae61c2d32ef3'}
...
if __name__ == '__main__':
    ...
    print('first prices:', minData[0][0]['close'],
          minData[1][0]['close'], minData[2][0]['close'])
