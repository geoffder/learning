...
def cross_entropy(T, pY):
    # it was just sum before..
    # return -(T * np.log(Y) + (1 - T)*np.log(1 - Y)).sum()
    return -np.mean(T*np.log(pY) + (1 - T)*np.log(1 - pY))  # mean instead
...
print("Final train classification rate:",
      classification_rate(Ytrain, np.round(pYtrain)))
print("Final test classification rate:",
