...
class Grid(object):
    ...
    def __init__(self, dims, walls, terminals, start, step_cost=0,
        ...
        self.move_list = ['U', 'D', 'L', 'R']
        self.windy = windy  # non-deterministic state transitions
        # states and actions
    ...
    def move(self, action):
        ...
        if self.windy:
            # half of the time get blown in one of the other directions
            prob = [.5/3 if i != self.move_list.index(action) else .5
                    for i in range(4)]
            action = np.random.choice(self.move_list, p=prob)
