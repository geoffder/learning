...
# GD note: This line would return a set of the unique sequence labels.
# numpy.asarray(list(i for i in set(map(tuple, Y_t))))
# For 12 bits, the number of unique labels is 4096.
...
def get_wikipedia_data(n_files, n_vocab, by_paragraph=False):
    ...
    if not os.path.exists(prefix):
        print(
            "Are you sure you've downloaded, converted, and placed the "
            + "Wikipedia data into the proper folder?")
        print("I'm looking for a folder called large_files, adjacent to the"
              + "class folder, but it does not exist.")
        print("Please download the data from https://dumps.wikimedia.org/")
        print("Quitting...")
        exit()

    input_files = [f for f in os.listdir(prefix)
                   if f.startswith('enwiki') and f.endswith('txt')]

    if len(input_files) == 0:
        print("Looks like you don't have any data files, or they're in the"
              + "wrong location.")
        print("Please download the data from https://dumps.wikimedia.org/")
        print("Quitting...")
        exit()
